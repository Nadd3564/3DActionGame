...
namespace Cradle.FM{
	public class SearchState : FSMState {
		...
		public SearchState(Transform[] wp)
		{
			...
			this.SetStateID(FSMStateID.Searching);
		}


		public override void Reason(Transform player, Transform npc)
		{
			if(this.LessThanCheckReach(Vector3.Distance(npc.position, player.position), 7.0f))
			{
		...
		public override void Act(Transform player, Transform npc)
		{
			...
			if(this.GreaterThanCheckReach(Vector3.Distance(npc.position, destPos), 50.0f))
			{
			...
			if(this.LessThanCheckReach(Vector3.Distance(npc.position, destPos), 0.6f))
			{
				Debug.Log("Reached to the destination point/ncalculating the next point");
				this.FindNextPoint();
			}
